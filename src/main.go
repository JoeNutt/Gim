...
func enableRawMode() (*unix.Termios, error) {
	t, err := unix.IoctlGetTermios(stdin, ioctlReadTermios)
	if err != nil {
		return nil, err
	}
	...
	raw.Cc[unix.VMIN] = 0
	raw.Cc[unix.VTIME] = 1
	if err := unix.IoctlSetTermios(stdin, ioctlWriteTermios, &raw); err != nil {
		return nil, err
	}
	return t, nil
}
...
func (e *Editor) Close() error {
	...
	return unix.IoctlSetTermios(stdin, ioctlWriteTermios, e.origTermios)
}
...
func (e *Editor) MoveCursor(k key) {
	...
	// If the cursor ends up past the end of the line it's on put the cursor at the end of the line.
	var linelen int
	if e.cy < len(e.rows) {
		linelen = len(e.rows[e.cy].chars)
	}
...
// The number of times the user needs to press Ctrl-Q to quit  the editor with unsaved changes.
const quitTimes = 3
	...
	for y := 0; y < e.screenRows; y++ {
		...
		if filerow >= len(e.rows) {
			if len(e.rows) == 0 && y == e.screenRows/3 {
				welcomeMsg := fmt.Sprintf("Gim Text Editor editor -- version %s ", version, "made by Joseph-Nutt")
